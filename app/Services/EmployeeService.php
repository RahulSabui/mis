...
class EmployeeService
{
    ...
    public function activeEmployeelist($page, $limit, $spanId, $designationId)
    {
        ...
        $query = Employee::select(
            ...
            'employee_proccess_assignment.billableStatus',
            'employee_additional_info.noticeStatus',
            'employee_additional_info.serviceStatus',
            'employee_additional_info.istrainee',
    ...
    public function dataLogOfActiveEmployee($date, $spanId, $designationId)
    {
        
        $query = DB::table('active_employee_log')
            ->where('date', $date);

        if ($spanId) {
            $query->where('spanId', $spanId);
        }

        if ($designationId) {
            $query->where('designationId', $designationId);
        }

        $dataLog = $query->get();

        $queryspanCounts = DB::table('span')
            ->leftJoin('active_employee_log', 'span.id', '=', 'active_employee_log.spanId')
            ->select('span.name as span_name', DB::raw('count(active_employee_log.id) as employee_count'))
            ->groupBy('span_name') 
            ->orderBy('span.id', 'asc')
            ->where('active_employee_log.date', $date); 

        if ($spanId) {
            $queryspanCounts->where('active_employee_log.spanId', $spanId);
        }

        if ($designationId) {
            $queryspanCounts->where('active_employee_log.designationId', $designationId);
        }

        $spanCounts = $queryspanCounts->get();



        $querydesignationCounts = DB::table('designation')
            ->leftJoin('active_employee_log', 'designation.id', '=', 'active_employee_log.designationId')
            ->select('designation.id', 'designation.name as designation_name', DB::raw('count(active_employee_log.id) as employee_count'))
            ->groupBy('designation.id', 'designation.name')
            ->where('active_employee_log.date', $date)
            ->get();

        if ($spanId) {
            $querydesignationCounts->where('active_employee_log.spanId', $spanId);
        }

        if ($designationId) {
            $querydesignationCounts->where('active_employee_log.designationId', $designationId);
        }
        ...
        $billableCount = 0;
        $bufferCount = 0;
        $employeeUnderNoticeCount = 0;
        $traineeCount = 0;
        $probationCount = 0;
        $permanentCount = 0;
        $spanNameForGraph = [];
        $spanCountForGraph = [];
        $designationNameForGraph = [];
        $designationCountForGraph = [];



        foreach ($spanCounts as $key) {
            $spanNameForGraph[] = $key->span_name;
            $spanCountForGraph[] = $key->employee_count;

        }
        foreach ($querydesignationCounts as $key => $value) {
            // echo"<pre>";print_r($value);
            $designationNameForGraph[] = $value->designation_name;
            $designationCountForGraph[] = $value->employee_count;
        ...
        foreach ($dataLog as $key => $value) {
            ...
            // echo"<pre>";print_r($value->reportingId);
            if ($value->billable == "billable") {
                $billableCount++;
            }
            if ($value->billable == "buffer") {
                $bufferCount++;
            }
            if ($value->UnderNotice != "Not") {
                $employeeUnderNoticeCount++;
            }
            if ($value->isTrainee != 0) {
                $traineeCount++;
            }
            if ($value->Probation == "Permanent") {
                $permanentCount++;
            }
            if ($value->Probation == "Probation") {
                $probationCount++;
            }
        ...
        $data = [
            ...
            "underNoticeCount" => $employeeUnderNoticeCount,
            "traineeCount" => $traineeCount,
            "probationCount" => $probationCount,
            "permanentCount" => $permanentCount,
            "activeCount" => count($dataLog),
            "Emplist" => $dataLog,
            "list" => $empArr,
            "spanCounts" => $spanCounts,
            "spanName" => $spanNameForGraph,
            "spanEmployeeCount" => $spanCountForGraph,
            "designationName" => $designationNameForGraph,
            "designationEmployeeCount" => $designationCountForGraph,
        ];
